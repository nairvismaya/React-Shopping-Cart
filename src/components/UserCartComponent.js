  ...
  return (
    <div className={`cart${cartCourses.length > 0 ? "active" : ""}`}>
      ...
      {cartCourses.length == 0 ? (
        <p className="empty-cart">Geek, your cart is empty.</p> //if the cart is empty
      ) : (
        <div>
          <ul>
            {cartCourses.map(
              (
                item // if not then show the product
              ) => (
                            ...
                            onClick={(e) => {
                              setCartCourses((prevCartCourses) => {
                                const updatedCart = prevCartCourses.map(
                                  (prevItem) =>
                                    prevItem.product.id === item.product.id // if the specific product matches then increase or decrease logic
                                      ? {
                                          ...prevItem, //object
